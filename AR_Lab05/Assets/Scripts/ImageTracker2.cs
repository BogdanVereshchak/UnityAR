...
public class ImageTracker2 : MonoBehaviour
{
    ...
    [Tooltip("Чи повинен контент завжди повертатися до камери?")]
    [SerializeField] private bool faceCamera = true;
    [Tooltip("Швидкість плавного повороту за камерою")]
    ...
    void Start()
    {
        // Кешуємо камеру один раз, щоб не навантажувати телефон кожного кадру
    ...
    void Update()
    {
        // Логіка повороту до камери (Billboard)
        ...
        foreach (var content in spawnedContent.Values)
        {
            ...
            {
                // Знаходимо позицію камери, але залишаємо висоту (Y) об'єкта незмінною
                ...
                // Напрямок, куди має дивитися об'єкт
                Vector3 directionToCamera = targetPosition - content.transform.position;
                ...
                if (directionToCamera != Vector3.zero)
                {
                    ...
                    // Плавний поворот (щоб об'єкт не "смикався" різко)
                    content.transform.rotation = Quaternion.Lerp(
